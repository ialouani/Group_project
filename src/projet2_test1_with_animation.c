...
int nombre_1=0;int compteur=0;
while(compteur<8){
if(tab_voisinage[compteur]==BLANC) nombre_1++;
compteur++;}

 int color=(w->t)[indice];
 if(color==0 && nombre_1==3) return 1;//ça matche avec la regle numero1.(0->BLANC)

int nombre_2=0;int compteur2=0;
while(compteur2<8){
if(tab_voisinage[compteur2]==BLANC) nombre_2++;
 compteur2++;}

 if(color==BLANC){
   if(nombre_2>3 || nombre_2<2) return 2;//appliquer la regle 2(blanc->0).
 }
...
for(int i=0;i<HEIGHT;i++){
  for(int j=0;j<WIDTH;j++){
	...
	coupleR2.xy[0]=i;
	coupleR2.xy[1]=j;
	ajout_file(&f,coupleR2);}}} //j'ai enfile toutes les modifications
...
//iteration..
  ...
  //printf("------------------------\n");
 int p=0;
    while(p<HEIGHT*WIDTH-limite_file){
      delete_file(&f);p++;}
    //   display_file(f);
    print_created_random_world2(&ww);k++;}
  return;}
...
int i=0;
while(i<9){
rules1.couleurs_voisins_utiles[i]=-2;i++;}
...
int kh=1;
 while(kh<=LEN_MAX+1){
 rules12.nouvelle_couleur[kh]=-1;kh++;}
int j=0;
while(j<9){
rules12.couleurs_voisins_utiles[j]=-2;j++;}
